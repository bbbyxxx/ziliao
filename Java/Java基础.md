...
String.intern是一种手动将字符串加入常量池的方法，如果在常量池中存在与调用intern方法的字符串等值的字符串，就直接返回常量池中相应字符串的引用，否则在常量池中复制一份该字符串，并将其引用返回。

可以通过-XX:StringTableSize来设置字符串池的map大小，字符串池内部实现为一个HashMap。intern的执行速度非常看，可以直接使用==进行比较，比equals去逐一比较要快得多；内存占用少。但如果单纯使用intern进行数据存储的话，则效率不如未使用intern方法。
...
4. 如果survovor的空间不足或经历15次Minor GC还能在新生代中存活的对象会通过分配担保机制被送入老年代。

   ```
   当survivor空间中相同年龄所有对象大小总和大于survivor空间的一半，年龄大于或等于该年龄的对象就可以直接进入老年代。
   ```
...
- 并发阶段会占用一部分线程而导致应用程序变慢，总吞吐量会降低
  ...
  ```
  CMS默认启动的回收线程数是（CPU数量+3）/4，当CPU在4个以上时，并发回收时垃圾收集线程不少于25%的CPU资源；当CPU不足4个时，那么对用户程序的影响就变得很大。
  ```

- 基于标记-清除算法实现的收集器，会产生大量的空间碎片

  ```
  -XX:UseCMSCompactAtFullCollection: 用于在CMS收集器顶不住要进行FullGC时开启内存碎片的合并整理过程，内存整理的过程是无法并发的，所以停顿时间不得不变长（默认开启）。
  -XX:CMSFullGCsBeforeCompacation: 用于设置执行多少次不压缩的FullGC后，跟着来一次带压缩的。
  ```

- CMS无法处理浮动垃圾，CMS并发清理阶段用户线程还在运行着，伴随程序运行自然就还会有新的垃圾不断产生，这一部分垃圾只能等到下一次GC再清理掉。

  ```
  在JDK1.6中，CMS收集器的启动阈值提升至92%
  ```

####  G1收集器

G1将整个堆分为多个大小相等的独立区域（Region）（每个Region都有一个对应的Remember Set），G1跟踪各个Region里面的垃圾堆积的价值大小，在后台维护一个优先列表，每次根据允许的收集时间，优先回收价值最大的Region。

#####  回收过程

1. 初始标记：标记GC Roots能直接关联到的对象，并且修改TAMS的值，让下一阶段用户程序并发运行时，能在正确可用的Region中创建新对象，需要停顿线程，但耗时很短
2. 并发标记：从GC Roots开始对堆中对象进行可达性分析，找出存活的对象，耗时比较长，但可与用户程序并发执行。
3. 最终标记：为了修正在并发标记期间因用户程序继续运作而导致标记产生变动的那一部分标记记录，并将变化记录在现场Remembered Set Log里面，最终合并到Remembered Set中，需要停顿线程，可并行执行。
4. 筛选回收：对各个Region的回收价值和成本进行排序，根据用户所期望的GC停顿时间来制定回收计划。

####  特点

1. 并行与并发：G1能利用多CPU、多核环境下的硬件优势来缩短STW的时间，部分其它收集器原本需要停顿的GC动作，G1仍然可以通过并发的方式继续执行。
2. 分代收集：能够采用不同的方式去处理新创建的对象和已经存活了一段时间、熬过了多次GC的旧对象。
3. 空间整合：G1是基于标记-整理算法实现的，从局部上来看是基于复制算法实现的。
4. 可预测的停顿：G1除了追求低停顿外，还能建立可预测的停顿时间模型，能够明确指定在一个长度为M的时间片段内，消耗在垃圾收集上的时间不超过N毫秒。
