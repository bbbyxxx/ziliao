...
G1将整个堆分为多个大小相等的独立区域（Region），G1跟踪各个Region里面的垃圾堆积的价值大小，在后台维护一个优先列表，每次根据允许的收集时间，优先回收价值最大的Region。
...
3. 最终标记：为了修正在并发标记期间因用户程序继续运作而导致标记产生变动的那一部分标记记录，并将变化记录在现场Remembered Set Log里面，最终合并到Remembered Set中（每个region都有一个对应的remember set），需要停顿线程，可并行执行。
