...
应该指定列为NOT NULL，除非真的想存储NULL，在MySQL中，还有空值的列很难进行查询优化，如果可能，尽量使用0、或者特殊的值来代表空值。

###  简单的数据类型更好

整型数据比起字符，处理开销更小，因为字符串的比较更复杂，在MySQL中应该用内置的时期和时间数据类型，而不是用字符串来存储时间，以及用整型来存储IP地址。
...
![ip_save](images/MySQL/ip_save.png)
...
首先从根节点进行二分查找，如果找到则返回对应节点的data，否则找相应区间的指针指向的节点递归进行查找，直到找到节点或找到null指针，前者查找成功，后者返回失败。
